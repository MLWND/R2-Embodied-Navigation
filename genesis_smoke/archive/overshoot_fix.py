...
# 升降: 0 -> 0.3
print(f"[of] 升降: {ramp_to(lift, [0.3])} (应≈0.3, 不过冲)")
# 腰部: 0 -> 0.5
print(f"[of] 腰部: {ramp_to(waist, [0.5])} (应≈0.5, 不过冲)")
# 头部: 0 -> 0.4
print(f"[of] 头部: {ramp_to(head, [0.4])} (应≈0.4, 不过冲)")
# 左臂 J1-J7: 逐个 0 -> 0.5
for i, d in enumerate(arm_l):
    r = ramp_to(d, [0.5], steps=60)
    print(f"[of] 左臂 J{i+1}: {r} (应≈0.5)")
print("[of] 完成")
