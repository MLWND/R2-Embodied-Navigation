...
class RobotController(Node):
    ...
    def robot_move_origin(self):
        ...
        if not self.global_speed_client.wait_for_service(timeout_sec=2.0):
            ...
            return
        # 设置速度
        speed_request = GlobalSpeedSet.Request(speed=60.0)
...
def main(args=None):
    ...
    try:
        ...
        left_joints = [0.0,0.0,0.0,-90.24,-75.77,-54.85,0.0]
        right_joints = [0.0,0.0,0.0,-90.24,-75.77,-54.85,0.0]
        if not controller.arm_move(left_joints, right_joints):
            controller.logger.error("手臂动作失败!")
            time.sleep(5)

        # 弯腰 1
        if not controller.line_move(angle=90.0, pos=0.0, head_angle=20.0):
            controller.logger.error("升降动作失败!")
            time.sleep(5)
        
        # 伸出位置1
        left_joints = [0.0,20.0,-10.0,-75.24,-75.77,-54.85,0.0]
        right_joints = [0.0,20.0,-10.0,-75.24,-75.77,-54.85,0.0]
        if not controller.arm_move(left_joints, right_joints):
            controller.logger.error("手臂动作失败!")
            time.sleep(5)

        # 手臂伸出位置2
        left_joints = [-88.05,26.73,51.61,-28.79,-76.88,-25.44,0.73]
        right_joints = [-88.05,26.73,51.61,-28.79,-76.88,-25.44,0.73]
        if not controller.arm_move(left_joints, right_joints):
            controller.logger.error("手臂动作失败!")
            time.sleep(5)

        # 手臂微调拿箱子位置1
        left_joints = [-88.05,18.08,51.61,-28.79,-76.88,-21.44,12.1]
        right_joints = [-88.05,18.08,51.61,-28.79,-76.88,-21.44,12.1]
        if not controller.arm_move(left_joints, right_joints):
            controller.logger.error("手臂动作失败!")
            time.sleep(5)
        
        # 手爪抓箱子
        left_joints_f =  [3676,14765,13776,13651,13141,200]
        right_joints_f = [3676,14765,13776,13651,13141,200]
        controller.rhand_use(left_joints_f,right_joints_f)
        time.sleep(1)

        # 手臂回收位置1
        left_joints = [-68.16,22.33,51.61,-42.40,-76.88,-21.44,19.81]
        right_joints = [-68.16,22.33,51.61,-42.40,-76.88,-21.44,19.81]
        if not controller.arm_move(left_joints, right_joints):
            controller.logger.error("手臂动作失败!")
            time.sleep(5)
        ...
        # 手臂回收位置2
        left_joints = [-3.39,32.02,64.33,-53.76,-76.88,-18.60,19.81]
        right_joints = [-3.39,32.02,64.33,-53.76,-76.88,-18.60,19.81]
        if not controller.arm_move(left_joints, right_joints):
            controller.logger.error("手臂动作失败!")
            time.sleep(5)
        ...
        # 放下逻辑
        # 升高位置
        if not controller.line_move(angle=0.0, pos=400.0, head_angle=0.0):
            controller.logger.error("升降动作失败!")
            time.sleep(5)
        # 举高箱子
        left_joints = [-98.27,32.02,64.33,-53.76,-76.88,-18.60,19.81]
        right_joints = [-98.27,32.02,64.33,-53.76,-76.88,-18.60,19.81]
        if not controller.arm_move(left_joints, right_joints):
            controller.logger.error("手臂动作失败!")
            time.sleep(5)
        # 放下弯腰
        if not controller.line_move(angle=40.0, pos=0.0, head_angle=30.0):
            controller.logger.error("升降动作失败!")
            time.sleep(5)
        ...
        # 手臂张开一点
        left_joints = [-98.27,37.47,64.33,-53.76,-76.88,1.6,19.81]
        right_joints = [-98.27,37.47,64.33,-53.76,-76.88,1.6,19.81]
        if not controller.arm_move(left_joints, right_joints):
            controller.logger.error("手臂动作失败!")
            time.sleep(5)
        
        # 手臂调整位置
        # left_joints = [-83.88,18.11,35.92,-46.06,-110.13,-37.95,19.81]
        # right_joints = [-83.88,18.11,35.92,-46.06,-110.13,-37.95,19.81]
        # if not controller.arm_move(left_joints, right_joints):
        #     controller.logger.error("手臂动作失败!")
        #     time.sleep(5)
        ...
        # 手臂收回位置1
        left_joints = [-83.88,18.11,35.92,-46.06,-140.13,-37.95,19.81]
        right_joints = [-83.88,18.11,35.92,-46.06,-140.13,-37.95,19.81]
        if not controller.arm_move(left_joints, right_joints):
            controller.logger.error("手臂动作失败!")
            time.sleep(5)
        ...
        if not controller.arm_move([0.0]*7, [0.0]*7):
            ...
            time.sleep(5)
            # continue
        print(f'------------------总花费时间：{time.time()-start_time}-------------------')
