...
class SimulatorRunner:
    ...
    def setup_isaacsim(self):
        ...
        self._simulation_app = SimulationApp(
            # A800 无 RT 核：渲染器强制 DLSS 上采样（aa/op 会被覆盖为 3），导致画面颗粒感。
            # 无法关闭 DLSS，改用 Quality 档（execMode=2）提高内部分辨率（0.5x→0.67x）
            {'headless': headless, 'anti_aliasing': 0, 'hide_ui': False, 'multi_gpu': False,
             'extra_args': ['--/rtx/post/dlss/execMode=2', '--/rtx-defaults/post/dlss/execMode=2']}
        )
